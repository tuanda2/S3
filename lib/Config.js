...
class Config extends EventEmitter {
    ...
    _getConfig() {
        ...
        const proxyUrl = envProxy || (config.proxy ? config.proxy.url : '');
        if (proxyUrl) {
            assert(typeof proxy === 'string');
            const { protocol, hostname, port, auth } = url.parse(proxyUrl);
            assert(protocol === 'http:' || protocol === 'https:',
                'bad config: protocol must be http or https in proxy url');
            ...
            if (port) {
                ...
                assert(!Number.isNaN(portInt) && portInt > 0, 'bad config: ' +
                    'proxy port must be a number greater than 0');
            }
            if (auth) {
            ...
            this.proxy.certFilePaths = {};
            if (config.proxy && config.proxy.certFilePaths) {
                const p = config.proxy;
                assert(typeof p === 'object' &&
                    typeof p.certFilePaths === 'object');
                assert((p.certFilePaths.key && p.certFilePaths.cert &&
                    typeof p.certFilePaths.key === 'string' &&
                    typeof p.certFilePaths.cert === 'string') ||
                    !p.certFilePaths.key && !p.certFilePaths.cert);
                assert((p.certFilePaths.ca &&
                    typeof(p.certFilePaths.ca) === 'string') ||
                    !p.certFilePaths.ca);
                const { key, cert, ca } = p.certFilePaths;
                const certPathsObj =
                    assertCertPaths(key, cert, ca, this._basePath);
                this.proxy.certFilePaths = certPathsObj;
            }
        }
