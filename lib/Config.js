...
function assertCertPaths(key, cert, ca, basePath) {
    const certPathsObj = {};
    if (key) {
        const keypath = (key[0] === '/') ? key : `${basePath}/${key}`;
        assert.doesNotThrow(() =>
    ...
    if (cert) {
        const certpath = (cert[0] === '/') ? cert : `${basePath}/${cert}`;
        assert.doesNotThrow(() =>
    ...
    if (ca) {
        const capath = (ca[0] === '/') ? ca : `${basePath}/${ca}`;
        assert.doesNotThrow(() =>
    ...
    _getConfig() {
        ...
        if (proxyUrl) {
            ...
            if (p) {
                ...
                assert((p.key && p.cert &&
                    typeof p.key === 'string' && typeof p.cert === 'string') ||
                    (!p.key && !p.cert));
                assert((p.caBundle && typeof(p.caBundle) === 'string') ||
                    !p.caBundle);
                const { key, cert, caBundle } = p;
                const certPathsObj =
                    assertCertPaths(key, cert, caBundle, this._basePath);
                this.outboundProxy.certFilePaths = certPathsObj;
            }
        }
