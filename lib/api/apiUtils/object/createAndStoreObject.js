...
function createAndStoreObject(bucketName, bucketMD, objectKey, objMD, authInfo,
    ...
    // create non-transient location field as 'PENDING' when the
    // location is transient and CRR is enabled, it will be set by the
    // CRR processor of preferred CRR target to the actual preferred
    // location.
    if(config.locationConstraintIsTransient(location)
       ...
       && metadataStoreParams.replicationInfo.status === 'PENDING') {
        metadataStoreParams.nonTransientLocation = 'PENDING';
    }
