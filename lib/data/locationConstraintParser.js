    ...
    Object.keys(config.locationConstraints).forEach(location => {
        ...
        if (locationObj.type === 'aws_s3' || locationObj.type === 'gcp') {
            ...
            if (config.outboundProxy) {
                const parsedUrl = url.parse(config.outboundProxy.url);
                const options = Object.assign(
                    config.outboundProxy.certFilePaths, parsedUrl);
                options.keepAlive = true;
                connectionAgent = new ProxyAgent(options);
                httpOptions = {
                    proxy: config.outboundProxy.url,
                    agent: connectionAgent,
                    timeout: 0,
                };
            } else {
                connectionAgent = new https.Agent({ keepAlive: true });
                httpOptions = { agent: connectionAgent, timeout: 0 };
            }
